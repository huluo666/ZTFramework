/*
 ...
 Version        : 1.0
 Copyright      : 同城
 Description    : ZTUDID declaration
 ============================================================================
 */
...
+ (NSString *)UniquelyIdentifies {
    ...
    if (!udid) {
        ...
        if (version >= 7.0) {
            udid = [ZTUDID _UDID_iOS7];
        }
        else if (version >= 2.0) {
            udid = [ZTUDID _UDID_iOS6];
        }
        
        [ZTUDID settUDIDToKeyChain:udid];
    }
...
+ (BOOL)settUDIDToKeyChain:(NSString*)udid
{
    ...
    if ([ZTUDID getUDIDFromKeyChain]) {        // there is item in keychain
        [ZTUDID updateUDIDInKeyChain:udid];
        [dictForAdd release];
        return YES;
    }
